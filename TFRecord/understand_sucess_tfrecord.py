...
# # data3里面包含train,test,txt（train.txt, test.txt）,train, test里面都是图片，txt是对应图片的标签
#
#
...
#                 # 并成两排（2行4列的numpy数组）
#                 temp = temp.transpose()
#                 # 矩阵求转置（现在变成了4行两列的数组，图像与相对应的标签在一行）
#                 np.random.shuffle(temp)
#                 # 把上面这个4×2的举证的行打乱顺序
#                 image_list = list(temp[:, 0])
#                 # 取出上面这个已经打乱顺序的矩阵的第一列，对应图形
#                 label_list = list(temp[:, 1])
#                 # 取出上面这个已经打乱顺序的矩阵的第二列，对应标签
...
#     # 故下面这两个函数直接照搬tensorflow官网
#
# # 生成整数型的属性
# def int64_feature(value):#前面不要加短号啊，否则就变成是私有函数，后面无法访问了的
...
#
# # 以上程序可以将数据集中的所有训练数据存储到一个TFRecord文件中。
...
#
# def read_and_decode(tfrecords_file, batch_size,tfrecords_filename):
