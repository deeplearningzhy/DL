...
###############################################################
# def batchnorm(inputs,is_training,convolution=False):
#     beta = tf.Variable(tf.zeros([inputs.get_shape()[-1]]))
#     gama = tf.Variable(tf.ones([inputs.get_shape()[-1]]))
#     # pop_mean = tf.Variable(tf.zeros([inputs.get_shape()[-1]]), trainable=False)
#     # pop_variance = tf.Variable(tf.ones([inputs.get_shape()[-1]]), trainable=False)
#     epsilon=1e-5
#     ema=tf.train.ExponentialMovingAverage(decay=0.99)
#     if convolution:
#         batch_mean,batch_variance=tf.nn.moments(inputs,[0,1,2])
#     else:
#         batch_mean,batch_variance=tf.nn.moments(inputs,[0])
#
#     #############更新滑动平均###################################
#     def mean_and_vari_update():
#         update_op=ema.apply([batch_mean,batch_variance])
#         with tf.control_dependencies([update_op]):
#             return tf.identity(batch_mean),tf.identity(batch_variance)
#         #有了tf.identity()，控制流才起作用
#     ################################################################
#     m,v=tf.cond(is_training,mean_and_vari_update(),
#                 lambda:(ema.average(batch_mean),ema.average(batch_variance)))
#     norm_output=tf.nn.batch_normalization(inputs,mean=m,variance=v,scale=gama,offset=beta,variance_epsilon=epsilon)
#     return norm_output
...
def neural_net(x):
    with tf.variable_scope('neural_network'):
        ...
        # layer2=tf.layers.dropout(layer2,rate=0.8,training=is_training)
